...
import { buildCheckpointState, patchDescriptionWithState, parseDescriptionState } from '../workspace/checkpoint.js';
...
describe('ReviewOrchestrator', () => {
  ...
  describe('publishReply', () => {
    it('calls provider.postReply with marker', async () => {
      ...
      expect(mockProvider.postReply).toHaveBeenCalledWith(
        targetRef,
        'thread-1',
        expect.stringContaining('🤖 AI-generated via [revpack]'),
      );
  ...
  describe('prepare', () => {
    ...
    it('loads a provider-retained historical patch once per Thread Revision', async () => {
      const oldVersion: ReviewVersion = {
        ...mockVersion,
        versionId: 'v-old',
        baseCommitSha: 'old-base',
        headCommitSha: 'old-head',
      };
      const historicalThread = (threadId: string): ReviewThread => ({
        ...mockThread,
        threadId,
        position: {
          filePath: 'src/app.ts',
      ...
      (mockProvider.listAllThreads as ReturnType<typeof vi.fn>).mockResolvedValue([
        ...
        historicalThread('thread-old-2'),
      ]);
      (mockProvider.getDiffVersions as ReturnType<typeof vi.fn>).mockResolvedValue([mockVersion, oldVersion]);
      const getDiffVersionDiffs = vi.fn().mockResolvedValue([
        {
          oldPath: 'src/app.ts',
          newPath: 'src/app.ts',
          diff: '@@ -1,2 +1,2 @@\n context\n+published version',
      ...
      mockProvider.getDiffVersionDiffs = getDiffVersionDiffs;
      ...
      expect(firstThread).toContain('## Thread Context');
      expect(firstThread).toContain('published version');
      expect(secondThread).toContain('published version');
      expect(getDiffVersionDiffs).toHaveBeenCalledTimes(1);
      expect(getDiffVersionDiffs).toHaveBeenCalledWith(mockTarget, 'v-old');
    ...
    it('degrades only a positional thread when its Thread Revision is unavailable', async () => {
      ...
      expect(threadMd).toContain('provider did not return a historical diff or complete Thread Revision');
  ...
  describe('publishFinding', () => {
    ...
    it('uses Bitbucket-compatible markdown instead of HTML markers', async () => {
      currentBranchSpy.mockResolvedValue('feature/bitbucket');
      const createThread = vi.fn().mockResolvedValue('bb-thread-id');
      const bitbucketProvider = createBitbucketMockProvider({ createThread });
      const orchestrator = new ReviewOrchestrator({ provider: bitbucketProvider, workingDir: tmpDir });
      await orchestrator.prepare('21', 'workspace/repo');

      await orchestrator.publishFinding(
        {
          oldPath: 'README.md',
          newPath: 'README.md',
          newLine: 5,
          body: '_🟢 Nit_ | _documentation_\n\nReplace the placeholder heading.',
      ...
      const body = createThread.mock.calls[0][1] as string;
      expect(body).toContain('Replace the placeholder heading.');
      expect(body).toContain('###### 🤖 AI-generated via [revpack](https://github.com/stefanvictora/revpack)');
      expect(body).toContain('Model: Claude Fable 5');
      expect(body).not.toContain('<!-- revpack');
  ...
  describe('thread items in bundle', () => {
    it('includes thread items and aggregate digest', async () => {
      ...
      expect(result.bundleState.threads.digestVersion).toBe(4);
    });
  });
  ...
  describe('publishReview', () => {
    it('creates visible comment without advancing checkpoint when review content is non-empty', async () => {
      ...
      expect(noteBody).toContain('AI-generated via [revpack]');

      expect(mockProvider.updateDescription).not.toHaveBeenCalled();
    });

    it('uses Bitbucket-compatible markdown for review notes', async () => {
      currentBranchSpy.mockResolvedValue('feature/bitbucket');
      const createNote = vi.fn().mockResolvedValue('bb-note-id');
      const bitbucketProvider = createBitbucketMockProvider({ createNote });
      const orchestrator = new ReviewOrchestrator({ provider: bitbucketProvider, workingDir: tmpDir });
      await orchestrator.prepare('21', 'workspace/repo');

      const result = await orchestrator.publishReview('## Review\nPlease confirm rollout timing.', 'workspace/repo');

      expect(result).toEqual({ created: true, noteId: 'bb-note-id' });
      const body = createNote.mock.calls[0][1] as string;
      expect(body).toContain('Please confirm rollout timing.');
      expect(body).toContain('###### 🤖 AI-generated via [revpack](https://github.com/stefanvictora/revpack)');
      expect(body).not.toContain('<!-- revpack');
      expect(body).not.toContain('###### revpack:review-note');
      expect(body).not.toContain('<sub>');
    });
  ...
  describe('publishReviewBatch', () => {
    it('submits findings and review body via submitReview', async () => {
      ...
      const comments = submitReviewMock.mock.calls[0][1] as Array<{ body: string }>;
      expect(comments[0].body).toContain('AI-generated via [revpack]');
      expect(comments[0].body).toContain('Model: GPT-5.6');
    });
  ...
  describe('input validation', () => {
    it('publishReply throws when threadId is undefined', async () => {
      const orchestrator = new ReviewOrchestrator({ provider: mockProvider, workingDir: tmpDir });
      await expect(
        orchestrator.publishReply('!42', undefined, 'body', { kind: 'generation' }, 'group/project'),
    ...
    it('publishReply throws when body is undefined', async () => {
      const orchestrator = new ReviewOrchestrator({ provider: mockProvider, workingDir: tmpDir });
      await expect(
        orchestrator.publishReply('!42', 'thread-1', undefined, { kind: 'generation' }, 'group/project'),
