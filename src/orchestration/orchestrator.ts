...
export class ReviewOrchestrator {
  ...
  static readonly COMMENT_MARKER = '<!-- revpack -->';
  ...
  async prepare(ref?: string, defaultRepo?: string, options?: PrepareOptions): Promise<PrepareResult> {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('multiple revpack state blocks')) {
        throw err;
      }
    ...
    try {
      ...
      throw new Error(
        'Cannot prepare: not inside a git repository.\n' +
          'Run `revpack checkout` first to clone or switch to the MR source branch.',
      );
    }
  ...
  async publishReply(
    ...
    await this.provider.postReply(targetRef, resolvedId, this.wrapManagedComment(body, attribution));
  }
  ...
  async publishFinding(finding: NewFinding, defaultRepo?: string): Promise<string> {
    ...
    return this.provider.createThread(
      targetRef,
      this.wrapManagedComment(finding.body, generationPublishAttribution(finding.generation)),
      {
  ...
  async publishReview(visibleContent: string, defaultRepo?: string): Promise<{ created: boolean; noteId?: string }> {
    if (visibleContent.trim()) {
      const targetRef = await this.resolveRef(undefined, defaultRepo);
      const markedBody = this.wrapReviewNote(visibleContent.trim());
  ...
  async publishReviewBatch(
    ...
    const comments = findings.map((f) => {
      ...
      return {
        body: this.wrapManagedComment(f.body, generationPublishAttribution(f.generation)),
    ...
    if (this.provider.submitReview) {
      const targetRef = await this.resolveRef(undefined, defaultRepo);
      const body = reviewBody.trim() ? this.wrapReviewNote(reviewBody.trim()) : '';
  ...
  private localPatchFailure(baseSha: string, headSha: string, reason: string): Error {
    return new Error(
      `revpack could not generate the review patch from local Git.\n\n` +
        `Required range:\n` +
  ...
  private wrapManagedComment(body: string, attribution: PublishAttribution): string {
    const footer = renderPublishAttributionFooter(this.provider.providerType, attribution);
    if (this.provider.providerType === 'bitbucket-cloud') return `${body}${footer}`;
    return `${ReviewOrchestrator.COMMENT_MARKER}\n${body}${footer}`;
  }

  private wrapReviewNote(body: string): string {
    return `${body}${renderPublishAttributionFooter(this.provider.providerType, { kind: 'generation' })}`;
  }
}
