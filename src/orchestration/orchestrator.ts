...
export class ReviewOrchestrator {
  ...
  static readonly BITBUCKET_AI_FOOTER =
  ...
  async publishReply(ref?: string, threadId?: string, body?: string, defaultRepo?: string): Promise<void> {
    ...
    await this.provider.postReply(targetRef, resolvedId, this.wrapManagedComment(body));
  }
  ...
  async publishFinding(finding: NewFinding, defaultRepo?: string): Promise<string> {
    ...
    return this.provider.createThread(targetRef, this.wrapManagedComment(finding.body), {
  ...
  private wrapManagedComment(body: string): string {
    if (this.provider.providerType === 'bitbucket-cloud') {
      return `${body}${ReviewOrchestrator.BITBUCKET_AI_FOOTER}`;
    }
  ...
  private wrapReviewNote(body: string): string {
    ...
    return `${body}${REVIEW_NOTE_FOOTER}`;
  }
}
