...
describe('BitbucketCloudProvider review comments', () => {
  ...
  it('marks visible revpack footers as bot-origin for Bitbucket-compatible markdown', async () => {
    installFetch(() =>
      jsonResponse({
        values: [
          ...
          comment({
            id: 502,
            content: { raw: 'Review body discussing the `AI-generated via [revpack]` footer.' },
            user: { display_name: 'Stefan Victora', nickname: null, account_id: 'human-user' },
          }),
          comment({
            id: 503,
            content: {
              raw: 'Generated reply\n\n###### 🤖 AI-generated via [revpack](https://github.com/stefanvictora/revpack)\n',
            },
            user: { display_name: 'Stefan Victora', nickname: null, account_id: 'human-user' },
          }),
          comment({
            id: 504,
            content: {
              raw: 'Direct reply\n\n###### Published via [revpack](https://github.com/stefanvictora/revpack)\n',
            },
            user: { display_name: 'Stefan Victora', nickname: null, account_id: 'human-user' },
          }),
        ],
    ...
    expect(threads.map((thread) => thread.comments[0].origin)).toEqual(['bot', 'human', 'bot', 'bot']);
  });
});
