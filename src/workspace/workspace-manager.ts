...
export class WorkspaceManager {
  ...
  private async buildContextTemplateView(
    ...
    // Derive SELF/REPLIED from revpack's hidden marker or terminal attribution footer.
    const selfThreadIds = new Set<string>();
    const repliedThreadIds = new Set<string>();
    for (const t of threads) {
      const nonSystem = nonSystemThreadComments(t);
      if (nonSystem.length > 0 && isRevpackAuthoredComment(nonSystem[0].body)) {
        selfThreadIds.add(t.threadId);
      } else if (nonSystem.some((c) => isRevpackAuthoredComment(c.body))) {
