...
export class WorkspaceManager {
  ...
  private async buildContextTemplateView(
    ...
    const hasRevpackCommentMarker = (body: string): boolean =>
      body.startsWith('<!-- revpack') || body.includes('Generated by [revpack]') || body.includes('via [revpack]');
...
const GENERATION_JSON_SCHEMA = {
  ...
  properties: {
    model: {
      ...
      maxLength: 80,
      pattern: '^[^\\r\\n]+$',
      description: 'Exact runtime model name, when exposed to the agent. Never guess or infer this value.',
    },
  },
...
const NEW_FINDINGS_JSON_SCHEMA = {
  ...
  items: {
    type: 'object',
    required: ['oldPath', 'newPath', 'body', 'severity', 'category', 'generation'],
    properties: {
      ...
      generation: GENERATION_JSON_SCHEMA,
    },
...
const REPLIES_JSON_SCHEMA = {
  ...
  items: {
    type: 'object',
    required: ['threadId', 'body', 'resolve', 'generation'],
    properties: {
