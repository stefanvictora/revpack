...
- Improved `revpack config setup` prompts so provider URLs are entered before provider selection, invalid URLs fail immediately, provider defaults are inferred from the URL, existing token environment variables are detected after creation, and invalid provider choices fail before later prompts.
