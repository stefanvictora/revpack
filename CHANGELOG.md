...
- Added TTY-only Guided Publish for bare `revpack publish`, with keyboard navigation, complete item previews, individual and contextual section-wide finding/reply selection, a final confirmation, preserved deferred drafts, and mandatory refresh before publishing a stale bundle; the selector uses an isolated terminal screen, redraws immediately on terminal resizes, keeps previews selection-neutral, renders common Markdown in outgoing review material with restrained terminal styling and visible link destinations while leaving unsupported syntax literal, shows compact positional context for findings and truncated originating thread context before replies, uses bold `Selection` and `Preview` pane headings and bold focused rows even when labels are truncated, organizes `New findings`, `Thread replies`, `Overview`, and `Review state` as consistently spaced peer sections with aggregate selection status, compactly indents selectable rows beneath non-focusable section headings so every selection has a useful preview, keeps focused rows aligned, separates stacked previews, lets stacked previews use the available height while the footer stays anchored, colors checkpoint warnings yellow, makes keyboard hints visually secondary, measures emoji and CJK text by terminal display width, and identifies deleted-file findings by their old paths; malformed queue drafts and unexpected review-note read failures remain visible, Windows JetBrains terminals are rejected with guidance to use Windows Terminal because their emulator can terminate the PowerShell session when the TUI closes, explicit subcommands stay non-interactive for scripts and CI, publishing uses the active bundle repository, primary `revpack publish note` plus `.revpack/outputs/note.md` provide review-note output with `publish review` as a hidden compatibility alias, legacy `review.md` drafts are not published, and explicit output discard removes them.
