...
- Positional thread Markdown reconstructs Thread Context from local Git. If the normal thread response already includes provider-rendered context, revpack keeps it only as a fallback; it does not make additional provider requests for historical patches. If neither local history nor embedded context is available, only the affected thread says so; inspect the checked-out source or `.revpack/diffs/latest.patch` for current code.
...
Bare `revpack publish` requires interactive stdin and stdout. It fails with explicit-command guidance when run without a terminal. On Windows, Guided Publish also refuses to start inside JetBrains terminals because their terminal emulator can terminate the PowerShell session when the TUI closes; run it in Windows Terminal instead. The [investigation record](troubleshooting/jetbrains-windows-guided-publish.md) preserves the reproduction, evidence, and criteria for removing this guard. Scripts and CI should use `revpack publish all` or a specific `revpack publish <command>`; these commands remain non-interactive and available in JetBrains terminals.
